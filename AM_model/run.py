...
def run(opts):
    ...
    if(opts.variation_type =='graph_size'):
        ...
        for i in [1, 2, 4]:
            for cur_graph_size in [20,30,50]:
                ...
                task_prop = {'graph_size': cur_graph_size, 'num_modes': num_modes, 'dist': 'gmm',
    ...
    print("alpha decay is ", opts.alpha_decay)
    ...
    for epoch in range(opts.epoch_start, opts.epoch_start + opts.n_epochs):
        ...
        if (epoch % 15 == 0):
            ...
            print('Saving last checkpoint model and state...')
            torch.save(
                {
                    'model': get_inner_model(model_meta).state_dict(),
                    'rng_state': torch.get_rng_state(),
                ...
                os.path.join(opts.save_dir, 'checkpoint.pt')
            )
