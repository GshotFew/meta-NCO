...
class TSPDataset(Dataset):
    ...
    def __init__(self, filename=None,num_samples=1000000, offset=0, distribution=None, task=None):
        ...
        if filename is not None:
            ...
            if (task['variation_type'] == 'tsplib'):
                #         num_points = int(split_line[1])
                self.data = self.process_mip_lib(filename)
                task['graph_size'] = len(self.data[0])
                pass
    ...
    def process_mip_lib(self, file_path):
        file_open = open(file_path, 'rb')
        pickle_file = pickle.load(file_open)
        torch_array = torch.from_numpy(pickle_file)
